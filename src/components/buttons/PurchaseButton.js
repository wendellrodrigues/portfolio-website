...
const Wrapper = styled.div`
  ...
  border-radius: 20px;
  display: grid;
  grid-template-columns: 53px auto; //Grid is set to columns. Detects two columns in react, set to 53 and auto
...
const Ring = styled.img`
  position: absolute; //auto aligns to entire browser
  top: -15px;
  left: -16px;

  ${Wrapper}:hover & { //& means self
    transform: rotate(30deg) scale(1.1) translate(0px, 2.5px);
`
...
const IconWrapper = styled.div`
  ...
  justify-self: center;
  position: relative; //Makes sure its children's positions defaults to this
